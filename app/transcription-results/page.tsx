    ...
    <>
      ...
      <main className="pt-20 min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-700 transition-colors duration-200">
        <div className="w-full h-full">
          ...
          <div className="grid grid-cols-1 gap-4 p-4 xl:grid-cols-3 xl:h-[calc(100vh-80px)]">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col min-h-[500px] xl:min-h-0">
              <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0 ml-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span>Duration: {hasDuration ? formatDuration(displayData.audio_duration) : 'Unavailable'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                ...
                {isVideoOnlyMode && (
                  <div className="mb-4 space-y-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
                    <div>
                      <p className="font-medium">Video loaded successfully.</p>
                      <p className="mt-1 text-amber-800 dark:text-amber-200">Transcript, notes, and insights are unavailable for this run.</p>
                    </div>
                    {failureReason && (
                      <div className="rounded-md bg-white/60 px-3 py-2 text-xs text-amber-900 dark:bg-black/10 dark:text-amber-100">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col min-h-[600px] xl:min-h-0">
              <div className="p-4 h-full flex flex-col">
                ...
                {isVideoOnlyMode ? (
                  <div className="flex-1 rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
                    <div className="flex h-full flex-col justify-between gap-4">
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">Transcript unavailable</p>
                        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                          This column is still part of the page layout, but there is no transcript data for this run yet.
                        </p>
                      </div>
                      <div className="space-y-3">
                        <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800/60">
                          ...
                          <p className="mt-1 text-sm text-gray-700 dark:text-gray-200">Waiting on a successful transcription.</p>
                        </div>
                        <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-800/60">
                          <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">When this succeeds</p>
                          <p className="mt-1 text-sm text-gray-700 dark:text-gray-200">Sentence timestamps, copy actions, and notes will appear here automatically.</p>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {EXPORT_FORMATS.map((format) => {
                          ...
                          return (
                            <button
                              key={format.name}
                              onClick={() => canExport ? handleExport(format.name, displayData) : alert(upgradeMessage)}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col min-h-[500px] xl:min-h-0">
              <div className="p-4 h-full flex flex-col">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  <MessageSquare className="w-5 h-5 text-primary-600" />
                  {isVideoOnlyMode ? 'Video Status' : 'Insights & Actions'}
                </h3>
                  ...
                  {isVideoOnlyMode ? (
                    <div className="space-y-4">
                      <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
                        <h4 className="text-sm font-medium text-gray-900 dark:text-white">Current State</h4>
                        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                          The player is available, but this run did not produce transcript data. That means transcript text, insights, and exports stay disabled until a transcription succeeds.
                        </p>
                      </div>

                      <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
                        <h4 className="text-sm font-medium text-blue-900 dark:text-blue-100">What You Can Verify Now</h4>
                        <ul className="mt-2 space-y-2 text-sm text-blue-800 dark:text-blue-200">
                          <li>The YouTube embed loads correctly.</li>
                          <li>The results page layout renders end-to-end.</li>
                          <li>The loading page now bypasses transcript failure and still lands here.</li>
                        </ul>
                      </div>

                      <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900/40">
                        <h4 className="text-sm font-medium text-gray-900 dark:text-white">Video ID</h4>
                        <p className="mt-2 break-all font-mono text-xs text-gray-600 dark:text-gray-300">{videoId || 'Unavailable'}</p>
                      </div>
                    </div>
                  ) : (
                    <>
